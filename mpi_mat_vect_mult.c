...
int main(int argc, char *argv[]) {
  ...
  int chunkSize = VECTOR_SIZE / world_size;

  int i;
  int * vector = (int*)malloc(sizeof(int) * VECTOR_SIZE);
  int * sub_row = (int*)malloc(sizeof(int) * chunkSize);

  if (world_rank == 0) {
    for (i = 0; i < VECTOR_SIZE; i++) {
      vector[i] = i;
    }
    printf("Vector Resulto:\n");
    pintar_Vector(vector);
  }

  MPI_Scatter(&vector, chunkSize, MPI_INT, &sub_row, chunkSize, MPI_INT, 0, MPI_COMM_WORLD);
  int shift = chunkSize * world_rank;

  for (i = 0; i < chunkSize; i++) {
    sub_row[i] += i + shift;
  }
  
  MPI_Gather(&sub_row, chunkSize, MPI_INT, &vector, chunkSize, MPI_INT, 0, MPI_COMM_WORLD);
...
void Read_n(
   ...
   if (my_rank == 0) {
      printf("¿Cual es el tamaño de los vectores?\n");
      scanf("%d", n_p);
   }
   MPI_Bcast(n_p, 1, MPI_INT, 0, comm);
   if (*n_p <= 0 || *n_p % comm_sz != 0) local_ok = 0;
   Check_for_error(local_ok, fname,
         "n debe ser > 0 y multiplo del número de procesos comm_sz", comm);
...
void Allocate_vectors(
   ...
   *local_z_pp = malloc(local_n*sizeof(double));
   ...
   Check_for_error(local_ok, fname, "No se puede asignar espacio para vector(es) local(es)", 
         comm);
}  /* Allocate_vectors */
...
float modulus_result( float x[] /* in */, float y[] /* in */, int n /* in */) {
      int i;
      float sum = 0.0;
